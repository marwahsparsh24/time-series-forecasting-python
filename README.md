...
You can find a more detailed Table of Contents on the main notebook (time-series-forecasting-tutorial.ipynb).

# Dataset

The dataset used is the Beijing air quality public dataset (https://archive.ics.uci.edu/ml/datasets/Beijing+PM2.5+Data). This dataset contains pollution data from 2014 to 2019 sampled every 10 minutes along with extra weather features such as pressure, temperature etc. We decided to resample the dataset with daily frequency for both easier data handling and proximity to a real use case scenario (predicting pollution 1 day ahead is more realistic than 10 minutes ahead). In this case, the series is already stationary with some small seasonalities which change every year.

In order to obtain an exact copy of the dataset used in this tutorial, please run the script under datasets/download_datasets.py (https://github.com/marwahsparsh24/time-series-forecasting-wiki/blob/master/datasets/download_datasets.py) which will automatically download the dataset and preprocess it for you.
...
# Contributing
Want to see another model tested? Do you have anything to add or fix? I'll be happy to talk about it! Open an issue or submit a pull request.

# Maintainer
This project is maintained by Sparsh Marwah, a Data Scientist with over 3 years of professional experience in machine learning and data engineering. My expertise includes building scalable pipelines using Python, SQL, and PySpark, with a focus on making complex modeling techniques accessible and reproducible.
